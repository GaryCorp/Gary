...
Gary manages three specialist AI employees. Each is a separate CrewAI agent
...
The Ethics Agent and CFO are future departments and are not part of this
version.
...
CrewAIExecutor                   one single-agent crew per assignment
  ↓
Susan / Dave / Linda             tool calls go through the ToolGateway
  ↓
...
| | Susan | Dave | Linda |
|---|---|---|---|
| Department | Research | Security | Operations |
...
Linda practical and unimpressed by unrealistic plans. Disagreement comes from
...
- **No delegation by employees.** CrewAI `allow_delegation=False` for all three;
...
Employees are advisory in this version: Dave's controls are recommendations,
...
`run_management_review` asks several employees (all three by default, or a
...
`failed`. Model reasoning is never stored.
...
docker compose exec backend python -m app.team_cli review "Give GaryCorp a browser automation capability" --agents susan,dave,linda
...
CrewAI, with the existing `OPENAI_API_KEY`. Susan's `web_search` uses OpenAI's
