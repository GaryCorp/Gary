-- Performance reviews are model calls and have to be priced like the rest.
--
-- Writing a review costs a model call, and the first live review was refused
-- by the ledger because 'performance_review' was not in the source CHECK: the
-- review was stored but its cost was not, which is exactly the kind of
-- unmeasured spending the ceiling exists to prevent.
--
-- SQLite cannot alter a CHECK constraint, so the table is rebuilt, the way
-- 010_audio_seconds.sql did for 'voice_transcription'.
...
INSERT INTO model_usage_new (
    ...
    audio_seconds, cost_usd, reported_cost_usd, entity_type, entity_id, detail, created_at
)
SELECT
    id, occurred_at, source, model,
    input_tokens, cached_input_tokens, output_tokens,
