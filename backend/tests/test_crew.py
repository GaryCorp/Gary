...
@pytest.mark.parametrize("agent_id", ["susan", "dave", "linda"])
...
def test_agents_are_separate_with_distinct_identities():
    agents = {agent_id: build_crewai_agent(request_for(agent_id), llm()) for agent_id in ("susan", "dave", "linda")}
    assert len({a.role for a in agents.values()}) == 3
    assert len({a.backstory for a in agents.values()}) == 3
